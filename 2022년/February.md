...
* 실패:
  ...
  * 2476(주사위 게임, *브3*): 코드를 지저분하게 짰으나 결국 해결하지 못함(내 제출 참고)
  ```java
  // 2차원 배열 StringTokenizer로 입력받기
...
* 성공하였으나 나중에 다시 볼 것(풀이 참조한 경우 등):
  * 16395(파스칼의 삼각형, **실5**): 나는 재귀로 풀었으나, 문제의 의도는 dp로 푸는 것이었다. dp로 다시 풀어보기, [참고](https://loosie.tistory.com/254)
...
* 성공하였으나 나중에 다시 볼 것(풀이 참조한 경우 등):
  * 11399(ATM, **실3**): [이 블로그](https://st-lab.tistory.com/147) 참고하면서 공부해보기.(카운팅 정렬 등) 발상 자체는 나도 옳게 했음.

#### 2월 23일
* 성공: 
* 실패:
  * 4796(캠핑, **실5**): 반례의 이유(틀린 이유)를 잘 모르겠음, '질문'의 반례에서, 입력이 1 3 5 가 들어오면, 정답이 2라고 한다(나의 실행값은 3임). 왜 2일까?
* 성공하였으나 나중에 다시 볼 것(풀이 참조한 경우):
...
* 실패:
* 성공하였으나 나중에 다시 볼 것(풀이 참조한 경우):
  * 2455(지능형 기차, **브3**): 나는 이차원 배열을 이용해서 쉽게 풀었으나, hashmap 사용해서 다음번에 도전해보기